...
2. Create a `ProviderConfig` that references a Secret with AAP **gateway root** URL (no `/api/controller` path; the embedded **ansible/aap** provider discovers the controller API via `GET {host}/api/`) and token (or username/password). Apply [deploy/providerconfig-default.yaml](deploy/providerconfig-default.yaml) on cluster, or use [provider/examples/](provider/examples/); create the Secret with [deploy/create-aap-credentials-secret.sh](deploy/create-aap-credentials-secret.sh).
...
| `deploy/providerconfig-default.yaml` | Default `ProviderConfig` for deploy (Secret `aap-credentials` in `crossplane-system`). |
