...
#        host: http://aap-gateway.aap-operator.svc.cluster.local
...
#   4. Run: ./deploy/testing-scripts/test-aap-credentials-aap-lab.sh
#   Non-CRC: if phase 1 (ingress) fails, set env AAP_SKIP_INGRESS_CHECKS=1 on the Job in validate-aap-api-suite-job.yaml.
#
set -euo pipefail
NS="${CROSSPLANE_NAMESPACE:-crossplane-system}"
JOB=validate-aap-api-suite
...
kubectl apply -f "$(dirname "$0")/validate-aap-api-suite-job.yaml"
echo "Waiting for job (up to 300s)..."
